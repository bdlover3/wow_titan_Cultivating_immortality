...
### 5.4 修仙宝典 (Cultivation Codex / Main Interface)

#### 5.4.1 功能描述
点击小师妹后打开的修仙宝典（原修仙录），整合所有修仙功能的入口。顶部显示境界、灵石、仙缘值状态信息。
...
│           ✦ 修 仙 宝 典 ✦                        │
├──────────────────────────────────────────────────┤
│  境界: 元婴期 ‖ 灵石: 123/45/67 ‖ 仙缘: 250      │  ← 状态栏
├──────────────────────────────────────────────────┤
│  ◆ 修仙根基 ◆                                     │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐
│  │  法  宝  │ │  机  缘  │ │  师  门  │ │  功  法  │
...
#### 5.4.3 Row 1 - 修仙根基（四大核心入口）

| 入口名称 | 对应功能 | 点击行为 |
|---------|---------|---------|
| 法宝 | 装备信息 | 打开角色装备界面 |
| 机缘 | 任务日志 | 打开任务日志 |
| 师门 | 宗门帮派 | 打开帮会面板（原"宗门"改为"师门"） |
| 功法 | 法术书 | 打开法术书 |

#### 5.4.4 Row 2 - 修仙百艺（生活技能）
...
**采集类**（点击打开采集图谱面板，显示各技能等级可采集的物品）:
- 灵植采集（草药）→ 显示草药采集图谱
- 灵矿开采（采矿）→ 显示矿石采集图谱
- 妖兽取材（剥皮）→ 显示皮革采集图谱

采集图谱面板显示每种植物的图标、名称、所需技能等级，已解锁的显示绿色✓，未解锁的显示红色✗。

#### 5.4.5 Row 3 - 仙途指引（功能入口）

| 入口名称 | 对应功能 | 点击行为 | 说明 |
|---------|---------|---------|------|
| 仙域冲突 | 随机战场 | 打开PvP面板 | 联盟显示"天界冲突"，部落显示"灵界冲突" |
| 小秘境夺宝 | 随机组队 | 打开副本查找器 | - |
| 道果录 | 修行成就 | 打开成就界面 | - |
| 灵宝鉴 | 灵宝图鉴 | 打开角色装备界面 | - |
| 神识探查 | 探查道友 | 打开神识探查面板 | - |
...
- `ToggleGuildFrame()` — 打开帮会面板
- `ToggleSpellBook("spell")` — 打开法术书
- `CastSpellByName(profName)` — 打开专业技能面板
- `ToggleAchievementFrame()` — 打开成就
- `TogglePVPFrame()` — 打开PvP面板
- `ToggleLFDParent()` — 打开副本查找器
...
### 5.8 仙缘值系统 (Immortal Destiny Value)

#### 5.8.1 功能描述
新增一套虚拟资源"仙缘值"，通过游戏行为自动获取，用于好感度送礼系统。仙缘值保存在 `WoWCultivationCharDB.immortalDestiny` 中。

#### 5.8.2 获取来源

| 来源 | 仙缘值 | 检测方式 |
|------|-------|---------|
| 击杀普通怪物 | 1-3 | COMBAT_LOG_EVENT_UNFILTERED / PARTY_KILL |
| 击杀精英怪物 | 5-12 | COMBAT_LOG_EVENT_UNFILTERED (检测精英标记) |
| 完成任务 | 8-20 | QUEST_COMPLETE 事件 / CHAT_MSG_SYSTEM |
| 采集灵材（草药/采矿/剥皮） | 2-5 | SKILL_LINES_CHANGED 检测技能等级提升 |
...
- 击杀怪物时通过 `bit.band(destFlags, 0x10)` 检测精英标记
- 采集通过对比技能等级快照检测等级变化
- Toast提示仅在非击杀来源时显示（击杀太频繁）
- 触发自定义事件 `IMMORTAL_DESTINY_CHANGED`

**API调用**: `COMBAT_LOG_EVENT_UNFILTERED`, `QUEST_COMPLETE`, `SKILL_LINES_CHANGED`, `GetNumSkillLines()`, `GetSkillLineInfo()`

### 5.9 好感度系统 (Favor System)

#### 5.9.1 好感度等级体系

为保持探索感，好感度**不显示具体数值**。界面只显示等级名称、称呼和进度条（无百分比数字）。
...
送礼消耗仙缘值而非金币。礼物tooltip显示消耗仙缘值数量，好感度增量显示为"未知（因人而异）"。
...
**偏好探索提示**: 灵儿对不同礼物的回应会揭示偏好线索（如"花...灵儿对花没什么感觉呢"暗示不喜欢花）

#### 5.9.4 对话选择系统

对话选项的好感值不显示，玩家根据灵儿的回应自行判断选择效果。
...
**API调用**: `WoWCultivationCharDB.immortalDestiny`（读取仙缘值）
**相关模块**: `ImmortalDestinyModule`, `FavorModule`, `FavorFrame`

### 5.10 道侣系统 (Dao Companion)
...
### 5.11 灵脉修炼 (Spirit Vein Cultivation)
...
│   ├── SectModule.lua          # 宗门/师门系统(职业→修仙门派)
...
│   ├── BattlegroundModule.lua  # 仙域冲突(战场→修仙化)
...
│   ├── KarmicDestinyModule.lua # 仙缘际遇系统(稀有事件记录)
│   ├── ImmortalDestinyModule.lua # 仙缘值系统(打怪/采集/任务获取)
│   ├── FavorModule.lua         # 好感度系统(小师妹送礼/对话)
...
Modules\ImmortalDestinyModule.lua
Modules\FavorModule.lua
...
UI\AlchemyFrame.lua
UI\CraftingFrame.lua
UI\AchievementFrame.lua
UI\TreasureFrame.lua
UI\MeditationFrame.lua
UI\DungeonFrame.lua
UI\SectWarFrame.lua
UI\OptionsFrame.lua
UI\MainFrame.lua
...
| 版本 | 功能范围 | 状态 |
|------|---------|------|
| v0.1.0 | 境界系统 + 宗门/师门系统 + 灵石系统 | 已完成 |
| v0.2.0 | 里程碑系统 + 小师妹引导 + 副职系统 | 已完成 |
| v0.3.0 | 仙域冲突 + 神识探查 + 修仙界频道 | 已完成 |
| v0.4.0 | 心魔系统 + 因果系统 + 仙缘际遇系统 | 已完成 |
| v0.5.0 | 修仙宝典主界面 + 修炼日志 + 丹药系统 | 已完成 |
| v0.6.0 | 仙缘值系统 + 好感度送礼 + 探索模式 | 当前版本 |
| v0.7.0 | 好感度深度互动 + 更多小师妹剧情 | 计划中 |
...
*文档版本: v0.6.0 | 最后更新: 2026-05-30 | 维护: XiuxianDev*
*新增: 仙缘值系统、好感度探索模式、修仙宝典主界面重构、师门系统*
